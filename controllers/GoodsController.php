...
class GoodsController extends BaseController
{
    ...
    public function actionDownload()
    {
        ...
        $tableHeader = ['品牌', '零件号', '中文描述', '英文描述', '原厂家', '厂家号', '材质', 'TZ', '加工', '标准', '询源', '错号', '大修',
            '锁定(是/否)', '保护', '铭牌', '所属设备', '所属部位', '建议库存', '设备用量', '单位', '技术', '原厂家备注', '零件备注', '发行含税单价',
...
//        exit();
        ...
        $tableHeader = ["品牌", "零件号", "是否有零件号", "是否有厂家号", "是否锁定", "是否有发行未税单价", "是否TZ", "是否总成", "是否加工", "是否标准", "是否询价", "是否有库存", "发行未税单价", "设备信息", "中文描述", "英文描述"];
        for ($i = 0; $i < count($tableHeader); $i++) {
            $excel->getStyle($letter[$i])->getAlignment()->setVertical('center');
            $excel->getStyle($letter[$i])->getNumberFormat()->applyFromArray(['formatCode' => NumberFormat::FORMAT_TEXT]);
            $excel->getColumnDimension($letter[$i])->setWidth(18);
        ...
        if (!isset($_FILES["FileName"])) {
            ...
            if ($_FILES["FileName"]["error"] > 0) {
                ...
                if (file_exists($saveName)) {
                    ...
                    //组装数据
                    $data = [
                        ["品牌", "零件号", "是否有零件号", "是否有厂家号", "是否锁定", "是否有发行未税单价", "是否TZ", "是否总成", "是否加工", "是否标准", "是否询价", "是否有库存", "发行未税单价", "设备信息", "中文描述", "英文描述"]
                    ];
                    foreach ($sheetData as $k => $v) {
                        if ($k > 1) {
                            $brand = trim($v['A']);
